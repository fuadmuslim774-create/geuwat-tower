...
export const GearInputRow: React.FC<GearInputRowProps> = ({
  ...
  return (
    <div
      className={`flex items-center ${isRight ? 'justify-end' : 'justify-start'} gap-4 min-h-[70px] relative`}
    >
      ...
      {/* Center Text - WELCOME/GEUWAT TOWER */}
      {!isActive && (
        <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 pointer-events-none">
          <span className={`text-1.5xl font-bold tracking-widest uppercase ${
            isRight ? 'text-fuchsia-500' : 'text-violet-500'
          }`}>
            {isRight ? 'WELCOME' : 'GEUWAT TOWER'}
          </span>
        </div>
      )}
